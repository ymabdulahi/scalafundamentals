...
object CurriesPAF extends App{
  ...
  List(1,2,3).map(x => inc(x))  // ETA-expansion - meaning the compiler turns the def method into a function. it turns List(1,2,3).map(inc) into List(1,2,3).map(x => inc(x))

  // Partial function applications
  val add5 = curriedAdder(5) _ // Int => Int ( it turns the method curriedAdder(5) into a function after applying in this case 5 to it.
